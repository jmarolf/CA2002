...
namespace CA2002
{
    public abstract class DoNotLockOnObjectsWithWeakIdentityAnalyzerBase : DiagnosticAnalyzer
    {
        ...
        {
            private INamedTypeSymbol? monitorTypeSymbol;
            private INamedTypeSymbol? marshalByRefObjectTypeSymbol;
            private INamedTypeSymbol? executionEngineExceptionTypeSymbol;
            private INamedTypeSymbol? outOfMemoryExceptionTypeSymbol;
            private INamedTypeSymbol? stackOverflowExceptionTypeSymbol;
            private INamedTypeSymbol? memberInfoTypeSymbol;
            private INamedTypeSymbol? parameterInfoTypeSymbol;
            private INamedTypeSymbol? threadTypeSymbol;
            ...
            private void ReportOnWeakIdentityObject(IOperation operation, OperationAnalysisContext context)
            {
                ...
                else if (operation.Type is ITypeSymbol type && TypeHasWeakIdentity(type, context.Compilation))
                {
                    context.ReportDiagnostic(operation.CreateDiagnostic(Rule, operation.Syntax.ToString()));
                }
            }
